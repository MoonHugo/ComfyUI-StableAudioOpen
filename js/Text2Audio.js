...
app.registerExtension({
    ...
    async nodeCreated(node) {
		if (node.title === "🔥stable-audio-open-1.0") {
			...
			loadLocalModelWidget.callback = function() {
				...
				// 重新计算并设置节点大小，同时将宽度增加1像素
				var newSize = [node.size[0] + 1, node.size[1]];
	...
	loadedGraphNode(node) {
		...
		if (node.type === "Text2Audio") {
			...
			if(node.widgets_values[10] != undefined){
				localModelPathWidget.value = node.widgets_values[10];
			}
